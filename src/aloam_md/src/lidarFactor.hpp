...
struct LidarPlaneFactor
{
		...
		: curr_point(curr_point_), last_point_j(last_point_j_), last_point_l(last_point_l_), last_point_m(last_point_m_), s(s_)
	{
		...
		ljm_norm.normalize(); ///Normalizes the vector, i.e. divides it by its own norm.
	}
	...
	static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_, const Eigen::Vector3d last_point_j_,const Eigen::Vector3d last_point_l_, const Eigen::Vector3d last_point_m_,
									   const double s_)
	{
		auto LidarPlaneFactorObj = new LidarPlaneFactor(curr_point_, last_point_j_, last_point_l_, last_point_m_, s_);
		auto AutoDiffCostFunctionObj = new ceres::AutoDiffCostFunction<LidarPlaneFactor, 1, 4, 3>(LidarPlaneFactorObj);
		return AutoDiffCostFunctionObj;
	}
